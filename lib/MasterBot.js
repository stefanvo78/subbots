...
function token(id, pass) {

  return new Promise((resolve, reject) => {
    var opts = {
      uri : "https://login.microsoftonline.com/botframework.com/oauth2/v2.0/token",
      body : `grant_type=client_credentials&client_id=${id}&client_secret=${pass}&scope=https%3A%2F%2Fapi.botframework.com%2F.default`,
      method : "POST"
    };
...
class MasterBot extends _UniversalBot {
  ...
  _sendToSub(credentials, req) {
    ...
    if (credentials.type === "sharedCredentials") {
      ...
      headers.authorization = this._tokenCache[credentials.appId];
