...
class MasterBot extends _UniversalBot {
 ...
 _isRouteableMessage(req) {
    ...
    return req.body.text && !req.body.text.startsWith("/");
  }
          ...
          if (topIntent.score > 0) {
            ...
            .then((response) => {
              if (response.statusCode >= 400) {
                this.requestLog += " - Dep failure - " + response.statusCode;
                console.warn("sendToSub:" + response.statusCode)
              }
            });
            res.end();
            next();
            this._lastRequestLog += " - Matched " + intentType.name;
            resolve(true);
          }
          else {
            resolve(false);
  ...
  _routeMessage(req, res, next) {
    ...
    if (req.body.conversation.id in this._focus) {
      this._lastRequestLog += " - Focus routed";
    ...
    .then((handled) => {
      if (!handled) {
        this._lastRequestLog += " - Default Route";
        this._defaultHandler(req, res, next);
      }
    });
  }
