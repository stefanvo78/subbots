...
class MasterBot extends _UniversalBot {
  ...
  startServer(config) {
    ...
    this._server.post(
      ':originalChannel/v3/conversations',
      this._createConversation.bind(this)
    );
  ...
  _getBearerToken(id, pass) {
    if (!(id in this._tokens) || this._tokens[id].expiry < Date.now()) {
      return new Promise((resolve, reject) => {
        this._refreshToken(id, pass)
        .then((response) => {
  ...
  _authenticatedRequest(opts, id, pass, cb) {
    this._getBearerToken(id, pass)
    .then((token) => {
      opts.headers = {
  ...
  _createConversation(req, res, next) {
    res.end();
    next();
  }
  ...
  _replyToActivity(req, res, next) {
    ...
    this._authenticatedRequest(opts, this._appId, this._appPassword, (err, response) => {
  ...
  _sendToSub(credentials, req) {
    ...
    if (credentials.type === "sharedCredentials") {
      req.body.serviceUrl = `https://12a7d549.ngrok.io/${req.body.channelId}`;
      //req.body.serviceUrl = `https://eyepeeaye-mainbot.azurewebsites.net/${req.body.channelId}`;
      ...
      return new Promise((resolve, reject) => { 
        ...
        this._authenticatedRequest(opts, credentials.appId, credentials.appPassword, (err, response) => {
          err ? reject(err) : resolve(response);
        });
      });
    }
  ...
  _tryRouteIntentType(req, res, next, intentType) {
    ...
    function filterIntents() {
      let intents = [];
      self._subs.forEach((sub) => {
        let subIntents = sub.intents.filter((i) => {
          return (i instanceof intentType);
        });
        intents.push({sub:sub, intents:subIntents}); 
      });
      return intents;
    }

    filterIntents(intentType).forEach((sub) => {
      sub.intents.forEach((i) => {
        tasks.push(i.score(req.body.text, sub.sub.credentials));
      });
    });
