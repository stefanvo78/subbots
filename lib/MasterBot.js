...
class MasterBot extends _UniversalBot {
  ...
  _postSubbots(req, res, next) {
    ...
    if (json) {
      ...
      this._subs.push(sub);
      if (sub.type == "directLine") {
        let directLine = new directLine(sub.credentials.secret);
        directLine.startConversation()
        .then((conversationId) => {
          res.send({conversationId : conversationId});
          res.end(200);
        });
      }
    }
  ...
  _sendToSub(credentials, req) {
    ...
    if (credentials.type === "sharedCredentials") {
      // Can't get the right grant_type to pull this off, move to directLine
